...
class LaterNewsCell: BaseTableViewCell {
    // MARK: - Outlet
    ...
    override func setDataContext(indexPath: IndexPath, data: BaseModel) {
        ...
        if let new = data as? NewsModel {
            ...
            if let url = new.getImageUrl(){
                self.titleImageView.kf.setImage(with: url, placeholder: R.image.no_image_bg_small())
            }else{
                self.titleImageView.image = R.image.no_image_bg_small()
            }
